#' Freesurfer ROI QC Interface
#'
#' Start the R Shiny with a Freesurfer ROI QC interface.
#'
...
#'
#' @export
freesurfer_qc = function(main_path){
  jsCode <- "
...
"
  ui = function(request) {
    ...
    fluidPage(
      ...
      titlePanel("Freesurfer Segmentation QC"),
      sidebarLayout(
        sidebarPanel(
  ...
  server = function(input, output, session) {
        ...
        if(length(rds_check) != 0){
          ...
          fluidRow(
            shinydashboard::box(
              ...
              textInput("file_save_path_com", "Save compiled QC file to:", value = paste0(dir_name, "/freesurfer_QC_result.csv")),
    ...
    output$welcome = renderUI({
      ...
      if(is.null(user_name)){
        tagList(list(
          ...
          tags$h2("Description"),
          HTML("<br>"),
          HTML("The Brain Segmentation QC Shiny App is a collaborative tool designed to facilitate the evaluation of brain segmentation masks generated by Freesurfer. It provides a user-friendly interface where multiple users can collectively assess roi segmentation quality through interactive features. <br>"),
    ...
    output$info = DT::renderDT({
      ...
      if(!is.null(info_df)){
        new_data = dataset()
        i = which(is.na(new_data[["evaluation"]]))[1]
        selected_column = c("subject", "session", "ICV")
        qc_list = result_list()
        dict_df = qc_list$dict
        roi_index = paste0(as.numeric(dict_df[which(dict_df$roi_general == input$lesion_id), "roi_index"]), collapse = ",")
        roi_name = input$lesion_id
        roi_volume = qc_list$stat[[i]][which(qc_list$stat[[i]]$SegId %in% as.numeric(dict_df[which(dict_df$roi_general == input$lesion_id), "roi_index"])), "Volume_mm3"] |> sum() |> round(2)
        info_df %>% filter(is.na(evaluation)) %>% head(1) %>% dplyr::select(selected_column) %>% mutate(roi_name = roi_name, roi_index = roi_index, roi_volume = roi_volume) %>%
    ...
    output$lesion_invest = renderUI({
      ...
      dict_df = qc_list$dict
      default_roi = unique(dict_df$default_roi)
      rois = unique(dict_df$roi_general)
      selectInput("lesion_id", "Select a roi to further investigate:", choices = rois, selected = default_roi)
    })
