        ...
        if(length(rds_check) != 0){
          ...
          fluidRow(
            shinydashboard::box(
              ...
              textInput("file_save_path_com", "Save compiled QC file to:", value = paste0(dir_name, "/lesion_QC_result.csv")),
    ...
    output$image = renderPapaya({
      ...
      if(!is.na(i)){
        ...
        if(new_data[i, "lesion_num"] > 0){
          ...
          }else{papaya(img=list(qc_list$summary_df$img_files[[i]], qc_list$labeled_lesion[[i]]==as.numeric(gsub("lesion ", "", input$lesion_id))), sync_view = TRUE,
                       hide_toolbar = FALSE, hide_controls = TRUE,
                       orthogonal = TRUE, options = list(papayaOptions(alpha = 0.5),
                                                         papayaOptions(alpha = 0.5, lut = "Red Overlay")))}
        }else{
