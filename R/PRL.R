#' PRL Lesion QC Interface
#'
#' Start the R Shiny with a PRL lesion QC interface.
#'
...
"
  ui = function(request) {
    ...
    fluidPage(
      ...
      titlePanel("PRL Segmentation QC"),
      sidebarLayout(
        sidebarPanel(
  ...
  server = function(input, output, session) {
        ...
        if(length(rds_check) != 0){
          ...
          fluidRow(
            shinydashboard::box(
              ...
              textInput("file_save_path_com", "Save compiled QC file to:", value = paste0(dir_name, "/PRL_QC_result.csv")),
    ...
    output$welcome = renderUI({
      ...
      if(is.null(user_name)){
        tagList(list(
          tags$h1("Welcome to our PRL Segmentation QC Shiny App"),
          tags$h2("Description"),
          HTML("<br>"),
          HTML("The PRL Segmentation QC Shiny App is a collaborative tool designed to facilitate the evaluation of PRL masks generated by automatic segmentation algorithms. It provides a user-friendly interface where multiple users can collectively assess PRL segmentation quality through interactive features. <br>"),
          tags$h2("Key Features"),
          HTML("<br>"),
          HTML("<strong>Subject Evaluation:</strong> Users can select pass or fail for each subject's PRL segmentation, enabling efficient evaluation of segmentation quality. <br><br>"),
          ...
          HTML("<br>"),
          HTML("<strong><span style='color: purple;'>Overall, the PRL Segmentation QC Shiny App empowers users to efficiently and collaboratively evaluate PRL segmentation quality, contributing to improved accuracy and reliability in neuroimaging research and clinical applications.</span></strong> <br>"),
          HTML("<br><br>"),
    ...
    output$info = DT::renderDT({
      ...
      if(!is.null(info_df)){
        ...
        info_df %>% filter(is.na(evaluation)) %>% head(1) %>% dplyr::select(selected_column) %>%
          ...
          prl_sub_df = prl_df[[i]] %>% filter(lesion_id == as.numeric(str_split(input$lesion_id, " ")[[1]][2]))
          info_df %>% filter(is.na(evaluation)) %>% head(1) %>% dplyr::select(subject, session) %>% mutate(lesion_id = input$lesion_id, rimpos = prl_sub_df$rimpos) %>%
            DT::datatable(options = list(columnDefs = list(list(className = 'dt-center',
                                                                targets = "_all"))))
        }
      }
