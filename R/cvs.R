#' CVS QC Interface
#'
#' Start the R Shiny with a CVS QC interface.
#'
...
"
        ...
        sidebarPanel(
          conditionalPanel(condition="input.tabselected==1",
                           fluidRow(
                             shinydashboard::box(
                               width = NULL,
                               title = "Lesion Center Mask Evaluation",
  ...
  server = function(input, output, session) {
    ...
    output$welcome = renderUI({
      ...
      if(is.null(user_name)){
        tagList(list(
          tags$h1("Welcome to our CVS Pipeline QC Shiny App"),
          tags$h2("Description"),
          HTML("<br>"),
          HTML("The CVS Pipeline QC Shiny App is a collaborative tool designed to facilitate the evaluation of lesion center masks for a better understanding of the CVS score. It provides a user-friendly interface where multiple users can collectively assess lesion center segmentation quality through interactive features. <br>"),
          tags$h2("Key Features"),
          HTML("<br>"),
          HTML("<strong>Subject Evaluation:</strong> Users can select pass or fail for each subject's lesion center segmentation, enabling efficient evaluation of segmentation quality. <br><br>"),
          ...
          HTML("<br>"),
          HTML("<strong><span style='color: purple;'>Overall, the CVS Pipeline QC Shiny App empowers users to efficiently and collaboratively evaluate lesion center segmentation quality, contributing to improved accuracy and reliability in neuroimaging research and clinical applications.</span></strong> <br>"),
          HTML("<br><br>"),
    ...
    output$info = DT::renderDT({
      info_df = dataset()
      qc_list = result_list()
      cvs_df = qc_list$cvs_score
      i = which(is.na(info_df[["evaluation"]]))[1]
      cvs_df = cvs_df[[i]]
      if(!is.null(info_df)){
        selected_column = c("subject", "session", "lesion_num")
        if(input$lesion_id == "all"){
        info_df %>% filter(is.na(evaluation)) %>% head(1) %>% dplyr::select(selected_column) %>% mutate(cvs.score = round(mean(cvs_df$cvs.score),3)) %>%
          DT::datatable(options = list(columnDefs = list(list(className = 'dt-center',
                                                              targets = "_all"))))
        }else{
          cvs_sub_df = cvs_df %>% filter(lesion_id == as.numeric(str_split(input$lesion_id, " ")[[1]][2]))
          info_df %>% filter(is.na(evaluation)) %>% head(1) %>% dplyr::select(subject, session) %>% mutate(lesion_id = input$lesion_id, cvs.score = cvs_sub_df$cvs.score) %>%
            DT::datatable(options = list(columnDefs = list(list(className = 'dt-center',
                                                                targets = "_all"))))
        }
      }
